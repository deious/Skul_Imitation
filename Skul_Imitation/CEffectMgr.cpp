...
void CEffectMgr::Add_Effect(const EffectInfo& tInfo, const Vec2& _vFinalPos)
{
    ...
    CObj* pEffect = Create_Effect(tInfo, vFinalPos);
...
CObj* CEffectMgr::Create_Effect(const EffectInfo& tInfo, const Vec2& vFinalPos)
{
    CObj* pEffect = new CEffect();

    dynamic_cast<CEffect*>(pEffect)->Initialize(tInfo);           // 모든 이펙트 정보 설정
